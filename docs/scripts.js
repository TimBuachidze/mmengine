// Все значения в пикселях в комментариях актуальны для браузера с шириной просмотра 1008px!
var temp;
var wrapperWidth;
...
function resize() {
	...
	if (screen.width > screen.height) {
		wrapperWidth = (document.body.offsetWidth * window.devicePixelRatio);
	}
	else {
		wrapperWidth = (document.body.offsetHeight * window.devicePixelRatio);
	}
	
	temp = document.getElementsByClassName('bodyblock');
	for ( i = 0; i < temp.length; i++ ) {
		temp[i].style.width = wrapperWidth - (wrapperWidth / 1008 * 50) + 'px';
		temp[i].style.marginTop = generateNominalPixels(10);
		temp[i].style.border = generateNominalPixels(15) + ' solid';
		temp[i].style.borderImage = 'url(images/border.png) repeat';
		temp[i].style.borderImageSlice = '20';
	};
	...
	document.getElementById('header-text-download').style.fontSize = generateNominalPixels(30);
	document.getElementById('header-text-download').style.paddingLeft = generateNominalPixels(40);
	document.getElementById('header-text-manual').style.fontSize = generateNominalPixels(30);
	document.getElementById('header-text-manual').style.paddingLeft = generateNominalPixels(40);
	document.getElementById('header-text-mods').style.fontSize = generateNominalPixels(30);
	document.getElementById('header-text-mods').style.paddingLeft = generateNominalPixels(40);
	...
	document.getElementById('speaker-knight').style.height = generateNominalPixels(252);
...
function pressButton(whichButton) {
	...
	if (buttonName != pressedButton) {
		...
		if (pressedButton == 'button-1') {
			document.getElementById('header-text-describe').style.display = 'table-cell';
		}
		else {
			document.getElementById('header-text-describe').style.display = 'none';
		}
		if (pressedButton == 'button-2') {
			document.getElementById('header-text-download').style.display = 'table-cell';
		}
		else {
			document.getElementById('header-text-download').style.display = 'none';
		}
		if (pressedButton == 'button-3') {
			document.getElementById('header-text-manual').style.display = 'table-cell';
		}
		else {
			document.getElementById('header-text-manual').style.display = 'none';
		}
		if (pressedButton == 'button-4') {
			document.getElementById('header-text-mods').style.display = 'table-cell';
			document.getElementById('header').style.background = 'url("images/footer-units.png") right bottom/auto 90% no-repeat';
		}
		else {
			document.getElementById('header-text-mods').style.display = 'none';
...
function generateNominalPixels(count) {
	return ( wrapperWidth / 1008 * count ) + 'px'
}
